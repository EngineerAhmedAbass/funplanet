...
app.controller('AllPage', function($scope, $http) {
	...
	var temp = "http://localhost:8080/Course";
    $http.get(temp).
        then(function(response) {
            $scope.items = response.data;
        });
    
    temp2 = "http://localhost:8080/Notify/"+studentmail;
    $http.get(temp2).
	...
	$scope.showgames=function(){
		...
		var temp = "http://localhost:8080/Game/Not/Course/"+val;
	    $http.get(temp).
	...
	{
		//alert(studentmail);
		var temp = "http://localhost:8080/Score/Student/"+studentmail;
		//alert(temp);
	    $http.get(temp).
