...
//    document.getElementById('confirmdiv').style.display = "block";
	...
	var temp = "http://localhost:8080/Teacher/"+val;
    $http.get(temp).
        then(function(response2) {
            ...
            if($scope.greeting.pass==val2)
            {
            	...
            	temp = "http://localhost:8080/Student/"+val;
            	$http.get(temp).
                then(function(response3) {
                    ...
                    if($scope.greeting2.password==val2)
                    {
                    	...
                    	window.location.href="http://localhost:8080/Student.html";
                    }
                    else{
	...
	$scope.sendcode=function(){
		...
		var tem="http://localhost:8080/Teacher/"+mail;
		$http.get(tem).
		...
		tem="http://localhost:8080/Student/"+mail;
		$http.get(tem).
       ...
       localStorage.setItem("Code",Code);
  	   ...
  	   var temp = "http://localhost:8080/Signip/"+Code+"/"+val;
       $http.get(temp).
            then(function(response3) {
           	 	//Confirm();
             })
		...
		if(code==Entered){
			...
			if(type=="Student"){
				...
				if(semail==undefined){
					...
					var res = $http.post('http://localhost:8080/Student', dataObj);
...
}else
	{
		...
		$http.post('http://localhost:8080/Teacher', dataObj);
		alert("Successfully Registerd ");
		window.location.reload();
