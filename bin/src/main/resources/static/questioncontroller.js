...
app.controller('AllPage', function($scope, $http, $timeout) {
	...
	var gamenamee="";
	var tempp="http://localhost:8085/Game2/Name/"+gameid;
	...
	if(gametype=="tf"){
		ShowTFQuestions();
		var temp = "http://localhost:8085/Questions/TF/"+gameid;
	    $http.get(temp).
		...
		ShowMSQQuestions();
		var temp = "http://localhost:8085/Questions/MSQ/"+gameid;
	    $http.get(temp).
    ...
    $scope.submittf=function(){
		...
		var res = $http.post('http://localhost:8085/Score', dataObj);
		showComment();
	...
	$scope.submitcomment=function(){
		...
		var res2 = $http.post('http://localhost:8085/Comment', dataObj2);
		if(usertype=="Teacher"){
